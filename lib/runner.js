...
(() => {
  ...
  async function doFill(adapter, betData, logger) {
    ...
    if (adapter.beforeFindMarket) {
      await adapter.beforeFindMarket(root, { sleep, waitFor, logger, betData });
    }
    ...
    if (adapter.fillStakeInput) {
      // For framework-reactive inputs (e.g. 22bet Vue 2) a native-setter write
      // doesn't update the model — the adapter routes the fill itself.
      await adapter.fillStakeInput(stakeInput, stakeValue, { clearFirst: adapter.clearStakeFirst ?? false, sleep });
    } else {
